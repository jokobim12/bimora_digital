...
export default function BottomNav({ active, onChange, musicPlaying, toggleMusic }: BottomNavProps) {
  ...
  return (
    <div className="w-full h-[70px] bg-transparent select-none relative flex items-end">
      ...
      {/* Left straight bar */}
      <div className="flex-grow h-[56px] bg-[#FAF6EC] border-t border-[#B38520]/30 flex justify-around items-center px-2">
        {leftTabs.map(tab => {
          ...
          return (
            <button
              ...
              className={`p-3 transition-all duration-300 relative cursor-pointer ${
                isActive ? 'text-[#B38520] scale-110' : 'text-[#6B5A43]/70 hover:text-[#3E2A00]'
              }`}
            >
              <IconComponent 
                className={`text-xl transition-all duration-300 ${
                  isActive ? 'drop-shadow-[0_0_8px_rgba(179,133,32,0.5)]' : ''
                }`} 
              />
              {isActive && (
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-[#B38520] rounded-full" />
              )}
            </button>
          )
        })}
      ...
      <div className="w-[100px] h-[56px] relative bg-transparent flex items-end shrink-0">
        ...
        >
          ...
          <path 
            ...
            fill="#FAF6EC" 
          />
          {/* Top border line using gold/bronze */}
          <path 
            d="M 0 0 
               C 18 0, 16 36, 50 36 
               C 84 36, 82 0, 100 0" 
            stroke="rgba(179,133,32,0.3)" 
            strokeWidth="1.2" 
            fill="none" 
          />
        </svg>
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-[-14px] z-20">
          <button
            onClick={toggleMusic}
            className={`w-11 h-11 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 border border-[#B38520]/45 active:scale-90 ${
              musicPlaying 
                ? 'bg-[#B38520] text-[#FAF6EC] shadow-[0_0_12px_rgba(179,133,32,0.4)]' 
                : 'bg-[#FAF6EC] text-[#B38520] hover:bg-[#F2EAD6]'
            }`}
