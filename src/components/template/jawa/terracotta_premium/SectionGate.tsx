...
export default function SectionGate({ children, id, className = '' }: SectionGateProps) {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(139,61,48,0.05)_0%,transparent_70%)] transition-opacity duration-1000 pointer-events-none z-10 ${
      ...
      <div className={`absolute inset-0 z-20 pointer-events-none flex overflow-hidden transition-opacity duration-[1000ms] ${
        ...
        {/* Left Gunungan Panel */}
        <div className={`absolute top-0 bottom-0 left-0 w-1/2 bg-[#FAF6EC]/95 border-r border-[#8B3D30]/20 flex items-center justify-end overflow-hidden transition-all duration-[1400ms] ease-out ${
          isOpen 
          ...
          <div className="absolute right-[-10px] top-1/2 -translate-y-1/2 w-[160px] h-[320px] md:w-[200px] md:h-[400px] overflow-hidden flex justify-end items-center">
            <img 
              ...
              alt="" 
              className="h-full w-auto max-w-none translate-x-[45%] object-contain opacity-40 filter drop-shadow-[0_0_12px_rgba(139,61,48,0.25)] rotate-[-12deg]" 
            />
          </div>
          {/* Subtle terracotta frame line on panel edge */}
          <div className="absolute top-0 bottom-0 right-1 w-[1px] bg-gradient-to-b from-transparent via-[#8B3D30]/20 to-transparent" />
        </div>

        {/* Right Gunungan Panel */}
        <div className={`absolute top-0 bottom-0 right-0 w-1/2 bg-[#FAF6EC]/95 border-l border-[#8B3D30]/20 flex items-center justify-start overflow-hidden transition-all duration-[1400ms] ease-out ${
          isOpen 
            ? 'transform translate-x-[110%] rotate-[12deg] opacity-0' 
            : 'transform translate-x-0 rotate-0 opacity-100'
        }`}>
          ...
          {/* Subtle terracotta frame line on panel edge */}
          <div className="absolute top-0 bottom-0 left-1 w-[1px] bg-gradient-to-b from-transparent via-[#8B3D30]/20 to-transparent" />
        </div>
      </div>
    </div>
  )
