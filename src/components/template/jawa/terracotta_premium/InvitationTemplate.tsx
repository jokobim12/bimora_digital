...
export default function InvitationTemplate({ data }: InvitationTemplateProps) {
  ...
  // Helper: send command to YouTube IFrame API
  ...
  // Imperatively drive HTML audio play/pause
  useEffect(() => {
  ...
  if (isLoading) {
    return (
      <div className="theme-jawa fixed inset-0 z-[150] bg-[#FAF6EC] flex flex-col items-center justify-center text-[#8B3D30] p-6">
        <div className="flex flex-col items-center max-w-sm text-center">
          <img 
            src="/assets/gunungan.png" 
            alt="Gunungan" 
            className="w-16 h-auto mb-6 filter drop-shadow-[0_0_12px_rgba(139,61,48,0.3)] sepia-[0.3] hue-rotate-[320deg] saturate-[1.8] animate-pulse" 
          />
          <p className="font-body text-[9px] tracking-[4px] uppercase text-[#8B3D30]/60 mb-2 font-bold">
            Serat Ulem
          </p>
          <h2 className="font-script text-xl text-[#5A1E17] tracking-wide font-bold">
            {data.groom_nickname} &amp; {data.bride_nickname}
          </h2>
          
          <div className="w-32 h-[1.5px] bg-[#8B3D30]/10 mt-6 relative overflow-hidden rounded-full">
            <div className="absolute inset-0 bg-[#8B3D30] animate-loading-bar" />
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="theme-jawa min-h-screen bg-[#F5EFE4] text-[#5A1E17] relative font-body selection:bg-[#8B3D30] selection:text-[#FAF6EC]">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden flex items-center justify-center">
        <img 
          src="/assets/gunungan.png" 
          alt="" 
          className="w-[100vh] h-auto object-contain opacity-[0.05] select-none sepia-[0.3] hue-rotate-[320deg] saturate-[1.8] animate-watermark" 
        />
      </div>

      {/* SIDE FLOATING GUNUNGAN DECORATIONS (Desktop Only) */}
      {isOpen && (
        <div className="hidden lg:block">
          ...
          >
            <img 
              src="/assets/gunungan.png" 
              alt="" 
              className={`w-full h-auto object-contain opacity-35 filter drop-shadow-[0_0_15px_rgba(139,61,48,0.2)] sepia-[0.3] hue-rotate-[320deg] saturate-[1.8] transition-transform duration-500 ease-in-out ${
        ...
        <div className="min-h-[100dvh] h-[100dvh] md:h-auto bg-transparent text-[#5A1E17] flex flex-col items-center justify-center relative overflow-hidden z-10">
          ...
          <div className="fixed top-4 left-4 right-4 z-40 flex justify-between items-center pointer-events-none">
            <button
              ...
              className="hidden md:flex items-center gap-2 bg-[#FAF6EC]/90 hover:bg-[#FAF6EC] border border-[#8B3D30]/20 hover:border-[#8B3D30] text-[#8B3D30] text-xs font-semibold px-4 py-2 rounded-full pointer-events-auto backdrop-blur-md transition-all active:scale-95 shadow-md"
          ...
          <div
            className={`w-full transition-all duration-500 ease-in-out ${
              isDesktopMode
                ? 'max-w-[1024px] rounded-2xl border border-[#8B3D30]/20 h-[calc(100vh-32px)] my-4'
                : 'max-w-[440px] h-[100dvh] md:h-[89vh] md:max-h-[810px] md:my-auto md:rounded-2xl md:border-2 md:border-[#8B3D30]/30'
            } bg-[#FAF6EC] flex flex-col relative overflow-hidden shadow-2xl`}
          >
            {/* Watermark in frame */}
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden flex items-center justify-center">
              <img 
                src="/assets/gunungan.png" 
                alt="" 
                className="w-[85%] max-w-[500px] h-auto object-contain opacity-[0.05] select-none sepia-[0.3] hue-rotate-[320deg] saturate-[1.8] animate-watermark" 
              />
            </div>
            ...
            <button 
              onClick={toggleMute}
              className="fixed right-6 bottom-24 lg:bottom-8 bg-[#FAF6EC] border border-[#8B3D30]/20 hover:border-[#8B3D30] text-[#8B3D30] rounded-full p-3.5 shadow-md active:scale-95 transition-all duration-300 z-50 cursor-pointer"
            ...
            >
              {isMuted ? (
                ...
                <div className="flex items-end justify-center gap-[3px] w-5 h-5 px-[1px] py-[2px]">
                  <span className="w-[3px] bg-[#8B3D30] rounded-full music-bar-1" />
                  <span className="w-[3px] bg-[#8B3D30] rounded-full music-bar-2" />
                  <span className="w-[3px] bg-[#8B3D30] rounded-full music-bar-3" />
                  <span className="w-[3px] bg-[#8B3D30] rounded-full music-bar-4" />
                </div>
              )}
            </button>
            ...
            >
              {/* PAGE 1: HOME */}
              <div ref={homeRef} className="border-b border-[#8B3D30]/10">
                <HomePage isDesktopMode={isDesktopMode} data={data} />
              </div>

              {/* PAGE 2: MEMPELAI */}
              <div ref={coupleRef} className="border-b border-[#8B3D30]/10">
                <SectionGate>
              ...
              {/* PAGE 3: ACARA */}
              <div ref={eventRef} className="border-b border-[#8B3D30]/10">
                <SectionGate>
              ...
              {/* PAGE 4: GALERI */}
              <div ref={galleryRef} className="border-b border-[#8B3D30]/10">
                <SectionGate>
              ...
              {/* FOOTER */}
              <div className="w-full text-center pt-6 pb-6 border-t border-[#8B3D30]/5 bg-[#FAF6EC] z-10 relative">
                <a 
                  href="https://instagram.com/jokobim12" 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="inline-flex items-center justify-center gap-1.5 font-body text-[10px] text-[#8B3D30]/60 hover:text-[#8B3D30] transition-all duration-300 cursor-pointer font-bold"
                >
            ...
            <div className={`absolute bottom-0 left-0 right-0 z-40 bg-[#FAF6EC] border-t border-[#8B3D30]/15 backdrop-blur-md px-4 py-2 ${
              isDesktopMode ? 'py-3' : ''
            }`}>
              <BottomNav 
            ...
            {/* Lightbox Popup */}
            <div 
              className={`absolute inset-0 z-[90] bg-[#FAF6EC]/95 flex items-center justify-center transition-all duration-300 ${
                lightbox ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
              }`}
              onClick={() => setLightbox(null)}
            >
              <button 
                className="absolute top-6 right-6 text-3xl text-[#8B3D30]/75 hover:text-[#8B3D30] hover:scale-110 transition-all cursor-pointer z-[100]"
              ...
              {lightbox && (
                <img 
                  className="max-w-[90%] max-h-[80vh] object-contain rounded-xl border border-[#8B3D30]/20 shadow-2xl animate-[zoomIn_0.3s_ease]" 
                  src={lightbox} 
      ...
      {/* Global Toast */}
      <div 
        className={`fixed left-1/2 -translate-x-1/2 z-[150] bg-[#FAF6EC] border-2 border-[#8B3D30]/50 rounded-2xl py-3.5 px-6 shadow-xl flex items-center justify-center gap-2.5 backdrop-blur-md w-[88%] max-w-[320px] transition-all duration-500 ease-out text-center ${
          showToast ? 'bottom-24 opacity-100 scale-100' : 'bottom-16 opacity-0 scale-95 pointer-events-none'
        }`}
      >
        <span className="font-body text-[10.5px] font-bold text-[#8B3D30] tracking-wide">
          {toastMsg}
        </span>
      </div>
    </div>
